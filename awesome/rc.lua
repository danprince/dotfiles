...
beautiful.init("/home/dan/.config/awesome/themes/chromebook/theme.lua")
...
-- {{{ Widgets
function read_file(path, format)
  format = format or "*l"
  mode = "r"
...
function BatteryWidget()
  ...
  function update()
    ...
    local charge_now  = tonumber(read_file(bat_dir .. "/charge_now"))
    local charge_full = tonumber(read_file(bat_dir .. "/charge_full"))
    local status = read_file(bat_dir .. "/status")
    local charge = math.floor((charge_now * 100) / charge_full)
    ...
    textbox.text = "<span color='" .. color .. "'>" .. charge .. "%</span>"

    return update
  end

  local refresh = timer({ timeout = 30 })
  refresh:add_signal("timeout", update())
  refresh:start();
...
for s = 1, screen.count() do
  ...
  mytasklist[s] = awful.widget.tasklist(function(c)
    -- don't pass the icon to hide from tasklist
...
-- {{{ Util
-- Functions for getting stuff done
